...
bool twoWayDrive = true;  // Single joystick operation mode or two
...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloud2)
{
  if (!useTerrainAnalysis) {
    ...
    laserCloudDwz->clear();
    laserDwzFilter.setInputCloud(laserCloudCrop);  // 降采样 尺寸在前面setLeafSize给出
  ...
  for (int i = 0; i < boundarySize; i++) {
    ...
    if (point1.z == point2.z) {
      ...
      for (int pointID = 0; pointID < pointNum; pointID++) {
        ...
        for (int j = 0; j < pointPerPathThre; j++) {  // pointPerPathThre=2 why add twice?
...
int main(int argc, char** argv)
{
  ...
  for (int i = 0; i < laserCloudStackNum; i++) {  // const int laserCloudStackNum = 1;
  ...
  while (status) {
    ...
    if (newLaserCloud || newTerrainCloud) {
      if (newLaserCloud) {
        ...
        laserCloudStack[laserCloudCount]->clear();  // why stack? maybe for multi lidar?
        *laserCloudStack[laserCloudCount] = *laserCloudDwz;  // laserCloudDwz: down sampled voxel around agent
      ...
      for (int i = 0; i < plannerCloudSize; i++) {
        ...
        point.x = pointX1 * cosVehicleYaw + pointY1 * sinVehicleYaw;  // Rotation Matrix
      ...
      while (pathScale >= minPathScale && pathRange >= minPathRange) {
        for (int i = 0; i < 36 * pathNum; i++) {  //pathNum for what?
          clearPathList[i] = 0;
          pathPenaltyList[i] = 0;
        }
        ...
        if (selectedGroupID >= 0) {
          ...
          int selectedPathLength = startPaths[selectedGroupID]->points.size();  // startPath is predefined
